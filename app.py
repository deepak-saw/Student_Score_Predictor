# ─────────────────────────────────────────
#  app_part1.py — ALL IMPORTS
#  Import this file's symbols in part2 & part3
# ─────────────────────────────────────────
...
#  (profile_widget.py contents)
#  Usage: from app_part2 import show_profile_widget
# ─────────────────────────────────────────

from app_part1 import *   # brings in st, json, os, etc.
...
.profile-avatar {
    ...
    cursor: pointer;
    border: 2px solid rgba(0,255,213,.4);
...
.pd-name {
    font-family: 'Syne', sans-serif;
    font-size: 17px; font-weight: 700; color: #dce3f0;
    margin-bottom: 3px;
}
.pd-role-badge {
...
def show_profile_widget():
    """
    Call this at the TOP of any dashboard (student or parent) — before other content.
    It injects the CSS + renders the top-right profile button + popover.
    """
    ...
    if st.session_state.get("app_theme") == "light":
        ...
        .stApp::before { opacity:.3; }
        .stApp::after  { opacity:.15; }
        ...
        .dash-title, .dash-greeting, .profile-name,
        .pd-name { color:#1a1f36 !important; }
        </style>
        """, unsafe_allow_html=True)


# ─────────────────────────────────────────
#  Edit Profile Modal (private helper)
# ─────────────────────────────────────────
...
#  (student_dashboard.py + parent_dashboard.py combined)
#  Usage: from app_part3 import show_student_dashboard, show_parent_dashboard
# ─────────────────────────────────────────

from app_part1 import *                         # st, joblib, pd, px, go, datetime, json, os, io
from app_part2 import show_profile_widget, load_json, save_json   # widget + helpers
    ...
    if st.button("🚀  Predict My Score"):
        ...
        with st.spinner("Generating PDF report..."):
            ...
            pdf_bytes = generate_pdf_report(
                ...
                school=school, internet=internet,
...
def show_parent_dashboard():
    ...
    for child in children:
        child_profile = students.get(child, {})
        last  = child_profile.get("last_score", "N/A")
        grade = child_profile.get("last_grade", "—")
        st.sidebar.caption(f"🎓 {child_profile.get('full_name', child)} — Score: {last} ({grade})")
    st.sidebar.markdown("---")
    if st.sidebar.button("🚪 Logout"):
        for k in ["logged_in", "username", "user_role"]:
            st.session_state[k] = False if k == "logged_in" else ""
    ...
    if st.button(f"🚀  Predict Score for {child_profile.get('full_name', selected_child)}", key=f"predict_{selected_child}"):
        ...
        with r2:
            ...
            bar_fig = px.bar(
                chart_data, x="Metric", y="Value",
                color="Value", color_continuous_scale=["#1a3a8f", "#0072ff", "#00ffd5"],
                title=f"{child_profile.get('full_name', selected_child)}'s Key Metrics",
                text="Value"
            )
            bar_fig.update_traces(marker_line_width=0, textposition='outside', textfont_color='#dce3f0')
            bar_fig.update_layout(
        ...
        with st.spinner("Generating PDF report..."):
            from app import generate_pdf_report
            pdf_bytes = generate_pdf_report(
                username=selected_child,
                student_profile=child_profile,
                hours=hours, attendance=attendance,
                previous=previous, sleep=sleep,
                motivation=motivation, teacher=teacher,
                school=school_sel, internet=internet,
                final_score=final_score, grade=grade, remark=remark
            )
